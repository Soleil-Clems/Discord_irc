  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleAddPrivateReaction(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    data: { messageId: number; emoji: string; conversationId: number },
  ) {
    try {
      const userId = await this.getUserIdFromSocket(client);
      if (!userId) return { error: 'Unauthorized' };

      const updatedMessage = await this.conversationsService.reaction(
        data.messageId,
        data.emoji,
        userId,
      );
      ...
      this.server
        .to(`user:${otherUser.id}`)
        .emit('privateReactionAdded', updatedMessage);
      this.server
        .to(`user:${userId}`)
        .emit('privateReactionAdded', updatedMessage);
